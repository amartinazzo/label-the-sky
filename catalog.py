...
def gen_master_catalog(catalogs_path, master_catalog_file, header_file='csv/fits_header_cols.txt'):
...
def gen_filtered_catalog(cat, dataset_csv, spectra_folder=None):
	c = pd.read_csv(cat)
	...
	c = c[['id', 'class','x','y','fwhm']]
	c.sort_values(by='id',inplace=True)
	c.to_csv(dataset_csv, index=False)


if __name__=='__main__':
	gen_filtered_catalog('csv/matched_cat_dr1.csv', 'csv/matched_cat_dr1_filtered.csv')
	...
	# query_sdss(spec_query, 'sdss_spec.csv')
