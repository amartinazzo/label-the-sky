...
def build_catalog_dataset(
        csv_file,
        features=['u', 'f378', 'f395', 'f410', 'f430', 'g',
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 4:
        print('usage: python %s <data_dir> <csv_file> <timestamp>' % sys.argv[0])
        exit(1)
    ...
    np.save(f'npy/exp00_Xcat_{timestamp}.npy', X_cat)
    ...
    np.save(f'npy/exp00_yy-val_catalog_logreg_{timestamp}.npy', yy_cat)
    ...
    train_classifier(
        ...
        clf_basepath + f'/exp00_catalog_{timestamp}.h5',
        class_weights)
    yy_cat = nn_cat.predict(X_cat['val'])
    np.save(f'npy/exp00_yy-val_catalog_nn_{timestamp}.npy', yy_cat)
    ...
    for n_bands in [3, 5, 12]:
        print(f'\n\n\nIMAGES {n_bands} BANDS')
        ...
        yy = lr.predict(X['val'])
        np.save(f'npy/exp00_yy-val_img-{n_bands}_logreg_{timestamp}.npy', yy)
        ...
        train_classifier(
            ...
            clf_basepath + f'/exp00_img-{n_bands}_{timestamp}.h5',
            class_weights)
        yy = nn.predict(X['val'])
        np.save(f'npy/exp00_yy-val_img-{n_bands}_nn_{timestamp}.npy', yy)
